...
var validationRequestEmailHtml = '<!DOCTYPE html><html>' + 
			...
			'<br>' + 
			'<p>Grassland curing data for Northern Territory is now ready for checking. To validate the ground observations, please log into ' + process.env.APP_NAME + ' ' + 
...
Parse.Cloud.define("sendEmailRequestForValidation", function(request, response) {
	...
	if (_IS_FIRE_DANGER_PERIOD) {
		...
		var data = {
			...
			cc: CFA_NEMP_EMAIL,
			from: CFA_NEMP_EMAIL,
			subject: "Northern Territory - Grassland Curing Validation Notification",
			text: "",
...
Parse.Cloud.define("sendEmailWelcomeNewUser", function(request, response) {
	...
	var html = '<!DOCTYPE html><html>' +
				...
				'<body>' + 
				'<p>Hi ' + firstname + ',' + '</p>' + '<br>' + 
				'<p>Thank you for participating in the Northern Territory grassland curing trial. This trial is supported in collaboration with the Fire and Rescue Service and is sponsored by the Commonwealth Attorney General&#39;s Department National Emergency Management Projects (NEMP).</p>' + '<br>' + 
				'<p>Currently in Victoria, grassland curing is monitored operationally using a combination of satellite data and field observations, which are reported weekly by observers using a web-based data entry tool. As a trial, we are deploying the Victorian approach for Northern Territory (as well as other states and territories). For online training videos, we encourage you to visit <a href="www.cfa.vic.gov.au/grass">www.cfa.vic.gov.au/grass</a>.</p>' + '<br>' + 
				'<p>The Northern Territory web-based data entry tool can be accessed via: <a href="' + GAE_APP_URL + '">' + GAE_APP_URL + '</a> (take note, the tool works best on Firefox, Chrome, Internet Explorer 9 or 10)</p>' + '<br>' + 
	...
	mailgun.messages().send({
	  ...
	  bcc: CFA_NEMP_EMAIL,
	  from: CFA_NEMP_EMAIL,
	  subject: "Welcome to the Northern Territory " + process.env.APP_NAME,
	  text: "",
	  html: html
    }, function (error, body) {
      if (error)
...
Parse.Cloud.define("sendEmailFinalisedDataToUsers", function(request, response) {
	...
	queryMMR.find({ useMasterKey: true }).then(function(results) {
		...
		var html = '<!DOCTYPE html><html>' +
					'<body>' + 
					'Hello all,' + 
					'<p>The Northern Territory grassland curing map has been updated for the ' + strToday + '. To view the map, please click <a href="' + GAE_APP_URL + '/viscaModel?action=grasslandCuringMap">here</a>.</p>' + 
					'<p>Kind Regards,</p>' + 
					'<p>The NEMP Grassland Curing Team <a href="' + CFA_NEMP_EMAIL + '">' + CFA_NEMP_EMAIL + '</a></p>' + 
					'<p><i>Note: This email has been generated automatically by ' + process.env.APP_NAME + '. Please do not reply to this email.</i></p>' + 
					'</body>' + 
		...
		mailgun.messages().send({
			...
			bcc: recipientList,
			from: CFA_NEMP_EMAIL,
			subject: "Northern Territory Grassland Curing Map - " + strToday,
			text: "",
			html: html
		}, function (error, body) {
			if (error)
